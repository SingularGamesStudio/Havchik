...
public class main : MonoBehaviour
{
									...
									if (razdel [i] == "quest") {
										...
										if (citnow.uns.Count > 0) {
											...
											if (!citnow.inbattleii) {
												if (!citnow.inbattleii) {
													if (((int)citnow.uns.Count * 5) >= teams [0].units.Count + 1 && citnow.race != -1) {
														...
														citnow.race = 11;
														citnow.exitii ();
														while (citnow.unitsin.Count * 0.33f > citnow.alldef.Count) {
															int yu = rnd.Next (citnow.unitsin.Count);
															citnow.alldef.Add (citnow.unitsin [yu]);
															citnow.unitsin.RemoveAt (yu);
														}
														citnow = null;
														dialog.SetActive (false);
							...
							if (citnow.d.act4 != "none") {
								...
								for (int i = 0; i < razdel.Count; i++) {
									if (razdel [i] == "quest") {
										...
										if (citnow.race != 11) {
												if (!citnow.inbattleii) {
											if (((int)citnow.uns.Count * 3) >= teams [0].units.Count + 1) {
												for (int j = 0; j < citnow.uns.Count; j++) {
													citnow.uns [j].SetActive (true);
												}
												teams [0].comgo.transform.position = citnow.sppos.transform.position;
	...
	public void startgame (bool ananas)
	{
		nowinobuch = false;
		lrncontroll.now = 8;
		lrncontroll.lrn1 (lrncontroll.g11,"Нажмите на кнопку слева для отдаления вида");
		if (ananas) {
