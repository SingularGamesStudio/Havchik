...
public class uniter : MonoBehaviour {
	...
	void Update () {
		...
		if(Mathf.Abs (Vector3.Distance (topresl.transform.position,gameObject.transform.position))>maxdist+main._m.teams[m].now*1.5f){
			gameObject.transform.Translate((new Vector3(gameObject.transform.position.x,gameObject.transform.position.y,0)-new Vector3(topresl.transform.position.x,topresl.transform.position.y,0))*-speed);
			if(curtimeout>maxer && !fly){
				a.Play();
				curtimeout=0;
			}
			if(Mathf.Abs (Vector3.Distance (topresl.transform.position,gameObject.transform.position))>(maxdist+main._m.teams[m].now)*20){
