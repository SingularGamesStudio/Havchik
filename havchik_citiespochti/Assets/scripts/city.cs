	...
	void Update () {
		if (main._m.ingame) {
			...
			if (curtimeout > 0.05f) {
				...
				if(inbattleii){
					if (Vector3.Distance (main._m.races[atk].com.army [0].comgo.transform.position, gameObject.transform.position) >= maxbatrasst) {
						if (main._m.races [race].com.nowin == num) {
							main._m.races [race].com.wait = rnd.Next (120);
							Destroy (main._m.races [race].com.army [0].comgo);

							for (int i = 0; i < main._m.races [race].com.army [0].units.Count; i++) {
								Destroy (main._m.races [race].com.army [0].inst [0]);
								main._m.races [race].com.army [0].inst.RemoveAt (0);
							}
							main._m.races [race].com.tselfor = "";
