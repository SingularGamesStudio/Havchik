...
public class racecommand : MonoBehaviour {
	...
	public void findtsel(){
		...
		List<string> todoin = new List<string> ();
		int a,sum=0;
		for (int i = 0; i < main._m.allcities.Count; i++) {
			...
			} else if (a + Mathf.Max (0, (0.33f * (main._m.allcities [i].unitsin.Count - a) - main._m.allcities [i].alldef.Count) / 1.33f) > 0) {
