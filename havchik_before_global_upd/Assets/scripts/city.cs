...
public class city : MonoBehaviour {
	...
	void Update () {
		if (main._m.ingame) {
			if (actmain && !nowhasarrow && Vector3.Distance (main._m.teams [main._m.chosedteam].comgo.transform.position, gameObject.transform.position) < 3) {
				nowhasarrow = true;
			...
			} else if (actmain && nowhasarrow && Vector3.Distance (main._m.teams [main._m.chosedteam].comgo.transform.position, gameObject.transform.position) >= 3) {
				nowhasarrow = false;
				Destroy (arrow);
			}
			if(dobsomething)
