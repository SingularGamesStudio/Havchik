...
public class main : MonoBehaviour
{
			...
			if (teams [i].casting) {
				...
				if (teams [i].curtimeout >= teams [i].casttime) {
					...
					if (allskills [p].deyst == "build") {
						...
						teams [i].unsdvig.Add(Vector3.zero); teams [i].units.Add (allskills [p].spawnnum);
		...
		if (ingame) {
					...
					if (chosingtsel) {
						if (!skilltsel && !posunit) {
							...
							if (allskills [waitfor [0]].deyst == "buildtsel") {
								...
								hh.transform.position = teams [chosedteam].comgo.transform.position;
								...
								teams [chosedteam].comgo.transform.position = h.transform.position;
							} else if (allskills [waitfor [0]].deyst == "sugarportal") {
								teams [chosedteam].comgo.transform.position = h.transform.position;
								if (teams [chosedteam].units.Count > 0) {
									for (int i = 0; i < teams [chosedteam].units.Count; i++) {
										teams [chosedteam].inst [i].transform.position = h.transform.position;
									}
								}
								resource [1] = 0;
							}
							...
							h.transform.localPosition *= sootv;
							h.transform.position = new Vector3 (h.transform.position.x, h.transform.position.y, 0);
							teams [chosedteam].unsdvig [topos] = h.transform.localPosition;
							chosingtsel = false;
							posunit = false;
					...
					if (teams [i].mainunit != -1) {
						...
						if (teams [i].mainunithp <= 0) {
							...
							if (teams [i].units.Count > 0) {
								...
								teams [i].unsdvig.RemoveAt (0); teams [i].units.RemoveAt (0);
			...
			if (curtimeout1 > 0.01f) {
				...
				if (SingleJoystick._s != null) {
					...
					teams [chosedteam].comgo.transform.Translate (v1 * speed * 2);
					teams [chosedteam].comgo.GetComponent<mainunit> ().gotsel = false;
				}
				//}
				if (creatingteam) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (teams [i].mainunit != -1) {
				if (Vector3.Distance (teams [chosedteam].comgo.transform.position, teams [i].comgo.transform.position) < combdist) {
					...
					teams [chosedteam].unsdvig.Add(Vector3.zero); teams [chosedteam].units.Add (teams [i].mainunit);
					...
					if (teams [i].units.Count > 0)
						for (int j = 0; j < teams [i].units.Count; j++) {
							...
							teams [chosedteam].unsdvig.Add(Vector3.zero); teams [chosedteam].units.Add (teams [i].units [j]);
	...
	public void add (int a)
	{
		if (creatingteam) {
			...
			if (!skilltsel) {
				posunit = true;
				topos = (a + sdvig) % (teams [chosedteam].units.Count);
				chosingtsel = true;
			}
		}
	}
	...
	public void endcreatingteam ()
	{
		...
		if (s1 == @"\supercheat") {
			...
			for (int i = 0; i < units.Count; i++) {
				...
				h.transform.position = teams [chosedteam].comgo.transform.position;
				h.GetComponent<uniter> ().m = chosedteam;
				h.GetComponent<uniter> ().num = i + 1;
				h.GetComponent<uniter> ().fly = units [i].fly;
				teams [chosedteam].unsdvig.Add(Vector3.zero); teams [chosedteam].units.Add (i + 1);
				teams [chosedteam].unitshp.Add (units [i].hp);
				teams [chosedteam].inst.Add (h);
				teams [chosedteam].now += 0.1f;
			}
			for (int i = 0; i < resource.Count; i++) {
			...
			if (a != -1 && newteamnums.Count > 0) {
				...
				if (newteamnums.Count > 1)
					for (int j = 1; j < newteamnums.Count; j++) {
						...
						teams [a].unsdvig.Add(Vector3.zero); teams [a].units.Add (teams [chosedteam].units [newteamnums [j]]);
			...
			for (int i = 0; i < 1000000; i = i) {
				if (teams [chosedteam].units [i] == -1) {
					...
					teams [chosedteam].unsdvig.RemoveAt (i);
				} else
					i += 1;
				if (i >= teams [chosedteam].units.Count)
					break;
	...
	public void townenter (GameObject town)
	{
		if (!onuimouse) {
			...
			if (Vector3.Distance (town.transform.position, teams [chosedteam].comgo.transform.position) < 3) {
				joy.SetActive (false);
				...
				if (citnow.d.usl1 != "") {
					...
					for (int i = 0; i < razdel.Count; i++) {
						if (razdel [i] == "quest") {
							...
							if (teams [chosedteam].mainunit != myparse (razdel [i + 1]))
								bo = false;
							i += 1;
						} else if (razdel [i] == "otn-") {
							if (races [citnow.race].otnosh >= myparse (razdel [i + 1]))
							...
							if (!teams [chosedteam].units.Contains (myparse (razdel [i + 1])) && teams [chosedteam].mainunit != myparse (razdel [i + 1]))
								bo = false;
							i += 1;
						} else if (razdel [i] == "unit-") {
							if (teams [chosedteam].units.Contains (myparse (razdel [i + 1])) || teams [chosedteam].mainunit == myparse (razdel [i + 1]))
	...
	public void ans (int num)
	{
		...
		if (trd) {
			if (tradestate == -1) {
				...
				talk.text += "Покупка: " + tobuycol.ToString () + " " + resnames [tobuy].ToString () + " за " + Mathf.Ceil ((rescourse [tobuy] * tobuycol / rescourse [tosell])).ToString () + " " + resnames [tosell].ToString () + "\n";
				...
				talk.text += "Покупка: " + tobuycol1.ToString () + " " + resnames [tobuy1].ToString () + " за " + Mathf.Ceil ((rescourse [tobuy1] * tobuycol1 / rescourse [tosell1])).ToString () + " " + resnames [tosell1].ToString () + "\n";
				...
				talk.text += "Подарить: " + togivecol.ToString () + " " + resnames [togive];
							...
							if (citnow.d.act1 != "none") {
								...
								for (int i = 0; i < razdel.Count; i++) {
									if (razdel [i] == "quest") {
										...
										if (teams [chosedteam].units.Contains (myparse (razdel [i + 1]))) {
											int o = teams [chosedteam].units.IndexOf (myparse (razdel [i + 1]));
											citnow.mainhouse.unitsin.Add (myparse (razdel [i + 1]));
											teams [chosedteam].unsdvig.RemoveAt (o); teams [chosedteam].units.RemoveAt (o);
											Destroy (teams [chosedteam].inst [o]);
											teams [chosedteam].inst.RemoveAt (o);
											teams [chosedteam].now -= 0.1f;
										} else if (teams [chosedteam].mainunit == myparse (razdel [i + 1])) {
											citnow.mainhouse.unitsin.Add (myparse (razdel [i + 1]));
											teams [chosedteam].mainunithp = -10;
										}
										i += 1;
									} else if (razdel [i] == "addunit") {
										if (citnow.mainhouse.unitsin.Contains (myparse (razdel [i + 1]))) {
											teams [chosedteam].unsdvig.Add(Vector3.zero); teams [chosedteam].units.Add (myparse (razdel [i + 1]));
											teams [chosedteam].unitshp.Add (units [myparse (razdel [i + 1]) - 1].hp);
											h = Instantiate (unitpref);
											h.GetComponent<uniter> ().race = 11;
										...
										if (!citnow.grabbed) {
											...
											for (int j1 = 0; j1 < citnow.unitsin.Count; j1++) {
												int yur = rnd.Next (5);
												if (yur == 1) {
													teams [chosedteam].unsdvig.Add(Vector3.zero); teams [chosedteam].units.Add (citnow.unitsin [j1]);
			...
			if (PlayerPrefs.GetString ("gamecreated1") == "1") {
				...
				for (int i = 0; i < 10; i++) {
					...
					if (razdel [g] == "1") {
						...
						h = Instantiate (compref);
						g += 1;
						teams [i].mainunit = myparse (razdel [g]);
						h.GetComponent<mainunit> ().num = myparse (razdel [g]);
						g += 1;
						teams [i].mainunithp = myparse (razdel [g]);
						h.GetComponent<mainunit> ().m = i;
						g += 2;
						...
						if (r != 0) {
							g += 1;
							for (int j = 0; j < r; j++) {
								teams [i].unsdvig.Add(Vector3.zero); teams [i].units.Add (myparse (razdel [g + j]));
							}
							g += r;
							for (int j = 0; j < r; j++) {
								teams [i].unitshp.Add (myparse (razdel [g + j]));
							}
							g += r - 1;
							for (int j = 0; j < teams [i].units.Count; j++) {
								h = Instantiate (unitpref);
	...
	public void startgame (bool ananas)
	{
		if (ananas) {
			...
			resource [0] = 5000;
			resource [1] = 1000;
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (1);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (1);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (1);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (5);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (5);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (2);
			...
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (5);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (5);
			teams [0].unsdvig.Add(Vector3.zero); teams [0].units.Add (6);
			teams [0].unitshp.Add (15);
			teams [0].unitshp.Add (18);
			teams [0].unitshp.Add (18);
			teams [0].unitshp.Add (18);
	...
	public void skilluse ()
	{
		if (choosedskill != -1) {
			...
			if (allskills [p].costunit == true) {
				if (!teams [chosedteam].units.Contains (allskills [p].unitc) && teams [chosedteam].mainunit != allskills [p].unitc) {
					canuse = false;
				}
			}
			if (canuse) {
				...
				if (allskills [p].costunit == true) {
					if (teams [chosedteam].units.Contains (allskills [p].unitc)) {
						int ju = teams [chosedteam].units.IndexOf (allskills [p].unitc);
						teams [chosedteam].units.RemoveAt (ju);
						teams [chosedteam].unitshp.RemoveAt (ju);
						teams [chosedteam].unsdvig.RemoveAt (ju);
						Destroy (teams [chosedteam].inst [ju]);
						teams [chosedteam].inst.RemoveAt (ju);
					}
					else
						teams [chosedteam].mainunithp = -10;
				}
